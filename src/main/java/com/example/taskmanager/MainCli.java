...
public class MainCli {
    ...
    private static void addN(Task task) {
        try {
            ...
            if (message.isEmpty()) {
                ...
                taskService.addNotificationToTask(task.getId(), reminderTime, message);
            }
    ...
    /**
     * Show Tasks with specified status.
     * <p>Status options: Total, Open, InProgress, Postponed, Completed, Delayed, DueIn7</p>
     */
    ...
    /**
     * Edit a task notifications (Add, Delete).
     * 
    ...
    private static void editNotification(Task task) {
        try {
            ...
            if (input.isEmpty()) {
                return; // User skipped editing notifications (enter pressed)
            }
